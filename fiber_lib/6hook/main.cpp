            ...
            while (true)
            {
                ...
                if (ret <= 0)
                {
                    ...
                    else if (errno == EAGAIN)
                    {
                        ...
                        //std::this_thread::sleep_for(std::chrono::milliseconds(50)); // 延长睡眠时间，避免繁忙等待（スリープ時間を延ばしてビジーウェイトを避ける）
                    }
                }
            }
        });
...
void test_iomanager()
{
    ...
    int yes = 1;
    // 解决 "address already in use" 错误（「アドレスが既に使用中」のエラーを解決する）
    ...
    // 绑定套接字并监听连接（ソケットをバインドして接続を待ち受ける）
