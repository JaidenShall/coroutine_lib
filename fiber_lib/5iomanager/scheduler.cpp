...
m_useCaller(use_caller), m_name(name)
{
	...
	if(use_caller)
	{
		...
		// スケジューラコルーチンを作成
		m_schedulerFiber.reset(new Fiber(std::bind(&Scheduler::run, this), 0, false)); // false -> このスケジューラが終了するとメインコルーチンに戻る
...
void Scheduler::run()
{
	...
	while(true)
	{
		...
		// 4 タスクなし -> アイドルコルーチンを実行する
		else
		{		
			// システム終了 -> idleコルーチンは無限ループから抜けて終了 -> idleコルーチンの状態はTERM -> 再度入るとループから抜けてrun()を終了
